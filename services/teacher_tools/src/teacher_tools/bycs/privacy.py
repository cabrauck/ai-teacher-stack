...
class ByCSPrivacyError(ValueError):
...
def _iter_items(payload: Any, prefix: str = "") -> Iterable[PrivacyIssue]:
    ...
    if isinstance(payload, dict):
        for key, value in payload.items():
            ...
            if is_forbidden_field_name(key_text):
                yield PrivacyIssue(
                    field_path=path,
                    field_name=key_text,
                    message=f"Field '{path}' is outside the ByCS v1 privacy boundary.",
                )
            if normalize_field_name(key_text) == "contains_personal_data" and value is True:
                yield PrivacyIssue(
                    field_path=path,
                    field_name=key_text,
...
def assert_export_privacy(*payloads: Any) -> PrivacyValidationResult:
    ...
    if not validation.ok:
        ...
        raise ByCSPrivacyError(f"ByCS export blocked by privacy policy: {joined}")
    return validation
